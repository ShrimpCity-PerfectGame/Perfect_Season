...
import { sget, sset, sdel, clearDraft } from "./storage.js";
...
// ---------- Storage ----------
// Shared (everyone):  acct:<user>  -> { username, salt, hash, created }
//                     stats:<user> -> season stats, best lineup, recent seasons
// Personal (device):  ps-session   -> { u, t } keeps you logged in on this device
const ACCT = "acct:";
const STATS = "stats:";
...
function applyRun(prev, run) {
  ...
  s.hist = addToHist(ensureHist(prev).hist, run.score);
  return s;
}
// Every account keeps a count of its seasons by team score (in tenths), so the whole site's
// distribution can be added up without anyone overwriting anyone else's data.
const histKey = (score) => String(Math.round(score * 10));
function addToHist(h, score) { const n = { ...(h || {}) }; const k = histKey(score); n[k] = (n[k] || 0) + 1; return n; }
function ensureHist(s) {
  if (!s || s.hist) return s;
  let h = {};
  (s.recent || []).forEach((r) => { if (!r.dnf) h = addToHist(h, r.score); });
  return { ...s, hist: h };
}
function placement(score, players, myKey, myStats, guestRun) {
  const k = Number(histKey(score));
  let above = 0, total = 0;
  const seen = new Set();
  const count = (h) => { for (const [b, n] of Object.entries(h || {})) { total += n; if (Number(b) > k) above += n; } };
  for (const q of players) {
    seen.add(q.id);
    count(q.id === myKey && myStats ? myStats.hist : ensureHist(q).hist);
  }
  if (myKey && myStats && !seen.has(myKey)) count(myStats.hist);
  if (guestRun) total += 1;
...
function AuthPanel({ onAuthed, title, blurb }) {
  ...
  async function submit() {
    ...
    if (!USER_RE.test(username)) return setErr("Usernames are 3 to 16 characters: letters, numbers, and underscores.");
    ...
    if (!window.crypto || !crypto.subtle) return setErr("This browser can't create secure logins. Try a current version of Chrome, Safari, or Firefox.");
    setBusy(true);
    const key = ACCT + username.toLowerCase();
    try {
      const existing = await sget(key, true);
      if (mode === "signup") {
        if (existing) { setBusy(false); return setErr(`${username} is taken. Try another username.`); }
        const { salt, hash } = await hashPassword(pw);
        const ok = await sset(key, { username, salt, hash, created: Date.now() }, true);
        if (!ok) { setBusy(false); return setErr("The account couldn't be created. Check your connection and try again."); }
        await onAuthed(username, hash, true);
      } else {
        if (!existing) { setBusy(false); return setErr("No account has that username. Check the spelling or create one."); }
        const { hash } = await hashPassword(pw, existing.salt);
        if (hash !== existing.hash) { setBusy(false); return setErr("That password is incorrect."); }
        await onAuthed(existing.username, hash, false);
      }
    } catch (e) {
      setErr("Something went wrong while logging in. Try again.");
    }
    setBusy(false);
  }
  ...
  return (
    <div className="panel">
      ...
      <div className="fields">
        <label>Username<input className="inp" value={u} maxLength={16} autoComplete="username" onChange={(e) => setU(e.target.value)} onKeyDown={onKey} /></label>
      ...
      {mode === "signup" && <p className="fine">Your username, best score, and best lineup appear on the leaderboard. Pick a password you don't use anywhere else.</p>}
    </div>
  );
}
...
const DAILY_SHARED = (d, u) => `daily:${d}:${u}`;
...
export default function PerfectSeason() {
  ...
  const [lb, setLb] = useState({ loading: true, players: [] });
  ...
  useEffect(() => {
    (async () => {
      ...
      if (sess && sess.u) {
        const acct = await sget(ACCT + sess.u, true);
        if (acct && acct.hash === sess.t) {
          setUser(acct.username);
          setStats(ensureHist((await sget(STATS + sess.u, true)) || blankStats(acct.username)));
        } else {
  ...
  async function loadLeaderboard() {
    ...
    try {
      const res = await window.storage.list(STATS, true);
      ...
      for (let i = 0; i < keys.length; i += 10) {
        const batch = await Promise.all(keys.slice(i, i + 10).map(async (k) => {
          ...
          return v ? { ...v, id: k.slice(STATS.length) } : null;
        }));
        players.push(...batch.filter(Boolean));
      }
      setLb({ loading: false, players });
    } catch (e) {
      setLb({ loading: false, players: [], error: true });
    }
  }
  ...
  async function saveStats(s) {
    ...
    const ok = await sset(STATS + key, s, true);
    setSaveError(!ok);
    setLb((x) => ({ ...x, players: [...x.players.filter((q) => q.id !== key), { ...s, id: key }] }));
  }

  async function onAuthed(username, hash, isNew) {
    const key = username.toLowerCase();
    await sset(SESSION, { u: key, t: hash }, false);
    let s = ensureHist((await sget(STATS + key, true)) || blankStats(username));
    const notes = [];
    if (isNew) {
  ...
  async function loadDailyBoard() {
    ...
    try {
      const d = todayKey();
      const res = await window.storage.list(`daily:${d}:`, true);
      const keys = (res?.keys || []).map((k) => (typeof k === "string" ? k : k.key)).filter(Boolean);
      const rows = [];
      for (let i = 0; i < keys.length; i += 10) {
  ...
  const place = finished && !lb.error ? placement(result.score, lb.players, myKey, stats, !user) : null;
          ...
          <>
            {lb.loading && lb.players.length === 0 ? (
              ...
              <>
                ...
                <div className="tiles">
                  <div className="tile"><div className="n">{lb.players.length}</div><div className="l">Players</div></div>
