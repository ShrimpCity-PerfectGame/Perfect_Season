...
await runTest("match_state reads the whole thing back, for a reload or a stranger", async () => {
  ...
  for (const who of [null, HOST, OTHER]) {
    ...
    // A reconnecting client rebuilds the boards and both rosters from these three (VERSUS.md 7), so none of
    // them can ever come back missing - replayMatch reads all three and would silently deal different boards.
    for (const list of ["respins", "dips", "steals"]) {
      assert(Array.isArray(state[list]) && state[list].length === 0, `${list} comes back as a list: ${JSON.stringify(state[list])}`);
    }
  }
  assert((await call(HOST, "match_state", { p_code: "nosuch" })).data === null, "and nothing for a code nobody has");

  // A steal comes back whole, and already in the sides replayMatch thinks in - there is nothing to translate,
  // which is the point of recording it on the match rather than by rewriting who a pick belonged to.
  await owner("update matches set steals = $1 where id = $2",
    [JSON.stringify([{ at: 4, by: "guest", pickNo: 1, slot: "FLEX1" }]), id]);
  const stolen = (await call(null, "match_state", { p_code: code })).data;
  assert(stolen.steals.length === 1 && stolen.steals[0].by === "guest" && stolen.steals[0].at === 4,
    `the steal reads back as it was written: ${JSON.stringify(stolen.steals)}`);
  // ...and the pick it took still says what it always said. match_picks is append-only.
  assert(stolen.picks[0].pickNo === 1 && stolen.picks[0].slot === "RB", "the pick row is untouched by it");
});
